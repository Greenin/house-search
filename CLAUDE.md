...
`house-search` es un clon arquitectónico de un proyecto hermano (`job-search`, no incluido en este repo) adaptado al dominio de búsqueda de casas en venta/alquiler. Corre **100% en local**, sin desplegar nada en la nube. Monorepo con tres partes independientes (`backend/`, `frontend/`, `scraper/`) más infraestructura en la raíz. Fase 1 (modelo de datos + backend CRUD + frontend navegable) ya está implementada; Fase 2 (scraping real) está en marcha con **pisos.com** como fuente funcional — ver más abajo por qué no es Idealista/Fotocasa/Habitaclia.
...
- **Dos modos de búsqueda, cada uno con su propia fila fija en `search_execution`** (`SearchExecution.ID_COMPLETA=1`/`ID_SIN_PLAYWRIGHT=2`, en vez de la fila única original): botón **"Ejecutar búsqueda"** (`POST /api/search/run`) lanza *todas* las fuentes activas y está limitado a una vez al día (protege el rate-limit de las fuentes que sí requieren Playwright); botón **"Búsqueda sin Playwright"** (`POST /api/search/run-sin-playwright`, `MODO_SCRAPER=SIN_PLAYWRIGHT` en el entorno del proceso hijo) lanza solo las fuentes con `requierePlaywright:false` (hoy, pisos.com) y **no tiene límite diario**. Ambos modos comparten el mismo lock de instancia en `SearchRunnerService` — no pueden correr a la vez, para no golpear la misma fuente por duplicado — pero el límite de "una vez al día" de un modo no afecta al otro, porque cada uno lee/escribe su propia fila.
- **Ambos botones borran toda la tabla `house` (y su `house_match_score`) antes de lanzar el scraper**, vía `HouseService.borrarTodasLasCasas()` — no solo las casas de la fuente que se va a re-ejecutar: "Casas encontradas" refleja siempre la última ejecución, no un acumulado histórico entre ejecuciones. `SearchRunnerService` lo invoca *dentro* del `synchronized` de `iniciarEjecucion` (parámetro `antesDeArrancar`), antes de lanzar el proceso hijo, para garantizar que el borrado termina antes de que el scraper pueda insertar nada. Efecto secundario asumido: si la ejecución falla o no encuentra nada, la tabla se queda vacía en vez de conservar los resultados de la ejecución anterior (decisión explícita, no un descuido). Las casas ya movidas a `SelectedHouse` nunca se tocan — es una copia deliberada del usuario, no parte de este listado.
...
Orquestador `index.js` con un array `FUENTES` que hoy solo contiene `{ nombre: 'Pisos.com', buscar: pisos.buscar, requierePlaywright: false }`. Cada fuente se ejecuta en su propio `try/catch` dentro del bucle de `main()`: si una fuente falla, las demás siguen (aislamiento por fuente). Si la variable de entorno `MODO_SCRAPER=SIN_PLAYWRIGHT` está definida (la fija `SearchRunnerService` al lanzar el proceso hijo desde el botón "Búsqueda sin Playwright"), `main()` filtra `FUENTES` a solo las que tengan `requierePlaywright: false` antes de ejecutar nada; sin esa variable (botón "Ejecutar búsqueda", o `node index.js` a mano) se ejecutan todas. `lib/insert.js` hace `POST /api/house/insert` con `X-API-Key`; `lib/profile-config.js` hace `GET /api/profile-config` (sin API key); `lib/normalize.js` tiene los helpers de normalización/deduplicación/filtrado compartidos; `lib/rate-limit.js` tiene `pausaEntrePaginas()` (2-3s aleatorios, compartido por todas las fuentes). `scraper/.env` es un symlink a `../.env`.

**Por qué pisos.com y no Idealista/Fotocasa/Habitaclia**: antes de implementar cada fuente se revisan su `robots.txt` real, sus condiciones de uso, **y una prueba real con el propio cliente `fetch()` del scraper** (no basta con `curl`: Habitaclia demostró que un mismo sitio puede responder distinto según el cliente). Idealista prohíbe expresamente "robot, spider, scraper o cualquier proceso automático" sin autorización escrita y lo vigila activamente con **DataDome** (confirmado); Fotocasa prohíbe la reproducción del contenido sin autorización y su `robots.txt` bloquea explícitamente `/search/`, `/buscar/` y las páginas de listado de las grandes ciudades. Ambas quedan como **stubs** (`sources/idealista.js`, `sources/fotocasa.js`, lanzan error si se invocan) deliberadamente fuera de `FUENTES`. **Habitaclia** tiene `robots.txt` permisivo y condiciones de uso sin prohibición expresa — se implementó y funcionó una vez — pero resultó estar protegida por **Imperva Incapsula**, que bloquea específicamente al cliente `fetch` del scraper con una pantalla de reto ("Pardon Our Interruption") aunque `curl` con la misma URL/user-agent la esquive; su código (`sources/habitaclia.js`) se deja intacto pero **fuera de `FUENTES`** hasta que se resuelva ese bloqueo — intentar burlar Imperva (rotar fingerprints, cabeceras, etc.) queda fuera de lo que se hace en este proyecto. **pisos.com** pasó las tres comprobaciones sin problema (`robots.txt` permisivo, sin prohibición de scraping, sin ningún reto anti-bot al probarla con `fetch()`) y es la fuente activa hoy.

**Dos mecanismos de acceso, elegidos por fuente**:
- **`lib/http.js`** (`descargarHtml`, `guardarHtmlError`) — para portales que sirven HTML ya renderizado en el servidor y no tienen anti-bot activo, como pisos.com (y Habitaclia, aunque hoy esté bloqueada): `fetch` simple con user-agent de navegador, sin arrancar Chromium. Más ligero y rápido.
- **`lib/browser.js`** (`abrirNavegador`, `capturarError`, `guardarSesion`) — Playwright, para futuras fuentes que sí necesiten JS real. `headless` se decide por la variable de entorno `HEADLESS`: **visible por defecto** (para depurar selectores a mano con `node index.js`), solo pasa a `headless:true` si `HEADLESS=true` está definida explícitamente (`SearchRunnerService` la fuerza al lanzar el proceso hijo). Sesión persistente opcional vía `context.storageState({ path: 'auth.json' })` — `scraper/auth.json` gitignorado; hoy ninguna fuente requiere login, queda listo para cuando haga falta. **No sirve para sortear anti-bot como Imperva/DataDome** — un navegador headless con Playwright sin más también es detectable; no es la solución al bloqueo de Habitaclia.
...
**`sources/pisos.js`** (activa): pide `https://www.pisos.com/{venta|alquiler}/pisos-<ubicacion>/` (sin paginar), extrae las tarjetas del listado (`div.ad-preview`, con `data-lnk-href` al detalle) y cada ficha individual para completar el resto de campos. A diferencia de Habitaclia, la ficha organiza cada característica en un `<div class="features__feature">` con un **icono estable** (`icon-ascensor`, `icon-terraza`, `icon-orientacion`, `icon-calefaccion`, `icon-aireacondicionado`, `icon-planta`, `icon-estadoconservacion`...) en vez de texto libre sin estructura — parseo por icono, más robusto que buscar palabras clave en frases. Certificado energético en `.energy-certificate__tag` (letra). Cada ficha va en su propio `try/catch`: si falla, la casa se conserva con los datos ya extraídos del listado y los campos de ficha a `null`. Tope de `MAX_RESULTADOS_POR_EJECUCION` (20) fichas por ejecución.

**`sources/habitaclia.js`** (implementada pero fuera de `FUENTES`, ver arriba): pide `https://www.habitaclia.com/{viviendas|alquiler}-<ubicacion>.htm`, parsea con `cheerio` los `<article class="has-aside">` de la ficha (texto libre e inconsistente entre anuncios, de ahí que esos campos sean `String` y no enum en el modelo de datos). Los enlaces de ficha se limpian de query string antes de pedirlos (`f=`, `geo=`, `from=`, `lo=`... están en `Disallow`). Mismo patrón de tope/aislamiento por ficha que `pisos.js`.

**Próxima fase**: revisar Fotocasa/otros portales si pisos.com no da suficiente volumen de resultados; Idealista queda descartado salvo que aparezca una vía de acceso autorizada (p. ej. su API de partners); Habitaclia se retoma solo si cambia su postura anti-bot.
